...
- **회원가입**: 처음 사용 시 새 계정 생성 (Windows/macOS 공유)
- **로그인**: 사용자명과 비밀번호로 로그인 (Windows/macOS 공유)
...
모든 데이터는 사용자 홈 디렉토리의 공유 폴더에 저장됩니다:

- **데이터베이스**: `~/MyTODO/todo.db` (사용자 계정, 할 일 목록) - 자동 생성
- **공유 폴더**: Windows와 macOS 간에 동일한 DB 공유

### 🔄 데이터 백업 및 복원

**백업 방법:**
1. `~/MyTODO/todo.db` 파일 복사
2. 또는 `~/MyTODO` 폴더 전체 복사

**복원 방법:**
1. 백업한 `todo.db` 파일을 `~/MyTODO/` 폴더에 복사
2. 앱 재실행

### 🌐 크로스 플랫폼 데이터 공유

- **Windows**: `C:\Users\사용자명\MyTODO\todo.db`
- **macOS**: `/Users/사용자명/MyTODO/todo.db`
- **동일한 계정**: Windows와 macOS에서 같은 사용자 계정으로 로그인 가능
- **실시간 동기화**: 한 플랫폼에서 변경하면 다른 플랫폼에서도 즉시 반영
